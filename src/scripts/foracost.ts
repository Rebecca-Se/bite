...
function healthCost(name): number {
...
if (phantom !== null) {
	let pmchp:number = getProperty(phantom,"data.attribute.hp.current");
	let pmthp:number = getProperty(phantom,"data.attribute.hp.temp");
	let pmmhp:number = getProperty(phantom,"data.attribute.hp.max");
