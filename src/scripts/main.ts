let casterName = null;
const js = "main.js";
let macro:Macro = Macro.config.baseEntity;
...
async function h(z:string):Promise<Boolean> {
...
form.addEventListener('click', async (event):Promise<Boolean> => {
	event.preventDefault();
	if (value.value != null)
  	value.value = c === 'cancel' ? null : form.nodeValue;
	if (c == null) {close(); Promise.resolve(true); return;}
	h(c);
