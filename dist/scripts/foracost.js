import { Actor } from "foundry-pc-types/types/core/config.js";
let casterName = null;
...
if (phantom !== null) {
    ...
    if (pmmhp - pmthp == pmchp && pmthp >= cost) {
        phantom.data.update("attribute.hp.temp", `${pmthp - cost}`);
        phantom.data.update("attribute.hp.max", `${pmmhp - cost}`);
    }
    else if (cost > pmthp) {
        let extra = parseInt(`${cost - pmthp}`);
        phantom.data.update("attribute.hp.max", `${pmmhp - cost + extra}`);
        phantom.data.update("attribute.hp.temp", `0`);
        phantom.data.update("attribute.hp.current", `${pmchp - extra}`);
    }
}
else {
    ...
    if (pmhp - pthp == pchp && pthp >= cost) {
        player.data.update("attribute.hp.temp", `${pthp - cost}`);
        player.data.update("attribute.hp.max", `${pmhp - cost}`);
    }
    else if (cost > pthp) {
        let extra = parseInt(`${cost - pthp}`);
        player.data.update("attribute.hp.max", `${pmhp - cost + extra}`);
        player.data.update("attribute.hp.temp", `0`);
        player.data.update("attribute.hp.current", `${pchp - extra}`);
    }
}
